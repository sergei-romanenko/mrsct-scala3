  ...
  override def descendants(g: G): List[G] =
    findBase(g) match
      ...
      case _ =>
        ...
        val driveSteps = if whistle.isEmpty then drive(g) else List()
...
trait UnaryWhistle[C] extends PFPTransformer[C]:
  ...
  override def inspect(g: G): Option[Warning] =
    if dangerous(g.current.conf) then Some(()) else None
  ...
  override def rebuildings(whistle: Option[Warning], g: G): List[G] =
    whistle match
      case None =>
        List()
      case Some(upper) =>
        val rebuilds = rebuildings(g.current.conf) map:
          rebuild(_)(g)
        val rollbacks = rebuildings(upper.conf) map:
          rollback(upper, _)(g)
        rollbacks ++ rebuilds
...
trait LowerAllBinaryGensOnBinaryWhistle[C]
  ...
  override def rebuildings(whistle: Option[Warning], g: G): List[G] =
    whistle match
      case None => List()
      case Some(upper) =>
        mutualGens(g.current.conf, upper.conf) map translate map:
          rebuild(_)(g)
...
trait UpperAllBinaryGensOnBinaryWhistle[C]
  ...
  override def rebuildings(whistle: Option[Warning], g: G): List[G] =
    whistle match
      case None => List()
      case Some(upper) =>
        mutualGens(upper.conf, g.current.conf) map translate map:
          rollback(upper, _)(g)
