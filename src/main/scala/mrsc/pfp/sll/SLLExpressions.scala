...
object Lifting:
  ...
  def expr2Task(e: Expr): SLLTask =
    ...
    def compare(n1: String, n2: String): Boolean =
      if n1.length < n2.length then true
      else if n1.length > n2.length then false
      else n1 < n2

    val defs1 = defs sortWith { (d1, d2) =>
      if compare(d1.name, d2.name) && compare(d2.name, d1.name) then
        val GFun(n1, Pat(pn1, _), _, _) = d1: @unchecked
        val GFun(n2, Pat(pn2, _), _, _) = d2: @unchecked
        pn1 < pn2
...
object SyntaxNormalization:
  ...
  def fixNames(e: Expr): Expr =
    ...
    def fixBoundVars(e: Expr, m: ListMap[Name, Int]): Expr =
      (e: @unchecked) match
        ...
        case Where(e1, defs) =>
          ...
          val defs1 = defs map:
            ...
            case GFun(name, Pat(n, pargs), args, body) =>
              val delta = (pargs ++ args).zipWithIndex map { case (k, v) =>
                (k, next + v)
              }
              val m1 = m ++ delta
              val body1 = fixBoundVars(body, m1)
              GFun(
                name,
                Pat(n, pargs map m1.andThen(vn)),
                args map m1.andThen(vn),
                body1
              )
