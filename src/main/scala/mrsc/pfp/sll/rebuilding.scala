...
object SLLRebuilding:
  ...
  def distinct(es: List[RawRebuilding[Expr]]): List[RawRebuilding[Expr]] =
    ...
    for e <- es do
      if !ls.exists(e1 => SLLSyntax.equiv(e1._1, e._1)) then ls += e
    ls.toList
  ...
  private def rebuild(
      ...
      e: Expr,
      sub: Subst[Expr]
  ): List[RawRebuilding[Expr]] =
    // rebuilding of arguments
    val rbs1 = e match
      case FCall(n, xs) =>
        rebuild1(top, xs, sub) map { case (ys, sub1) => (FCall(n, ys), sub1) }
      case GCall(n, xs) =>
        rebuild1(top, xs, sub) map { case (ys, sub1) => (GCall(n, ys), sub1) }
      case Ctr(n, xs) =>
        rebuild1(top, xs, sub) map { case (ys, sub1) => (Ctr(n, ys), sub1) }
      case _ => List((e, sub))
    // new rebuilding
    val rbs2 =
  ...
  private def rebuild1(
      top: Expr,
      es: List[Expr],
      sub: Subst[Expr]
  ): List[(List[Expr], Subst[Expr])] =
    es.foldRight((List[Expr](), sub) :: Nil) { (e, acc) =>
      for (es1, sub) <- acc; (t, sub1) <- rebuild(top, e, sub)
      yield (t :: es1, sub1)
    }
