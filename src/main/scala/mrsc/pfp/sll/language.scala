...
object SLLSyntax:
  ...
  private def walk(p: (Expr, Expr), s: Subst[Expr]): Option[Subst[Expr]] =
    p match
...
trait SLLDriving extends PFPTransformer[Expr] with Reducer:
  ...
  def caseGRedexCtr(ctx: Ctx, gcall: GCall, ctr: Ctr): List[GG] =
    ...
    val reduced =
      subst(body, ((p.args ++ gargs) zip (ctr.args ++ gcall.args.tail)).toMap)
    List(transientDriveStep(ctx(reduced)))
  ...
  def instantiate(p: Pat, v: Var): Ctr =
    val vars = p.args.indices.toList.map { i =>
      Var("de_" + p.name + "_" + i + "/" + v.name)
    }
    Ctr(p.name, vars)
