...
object Transformations:
  ...
  def transpose[C, D, E](g: SGraph[C, D]): TGraph[C, D] =
    ...
    val levels =
      orderedNodes.groupBy(_.sPath.length).toList.sortBy(_._1).map(_._2)
...
object GraphPrettyPrinter:
  def toString(node: TNode[?, ?], indent: String = ""): String =
    ...
    if node.base.isDefined then sb.append("*******")
    for edge <- node.outs do
      sb.append(
        "\n  " + indent + "|" + (if edge.driveInfo != null then edge.driveInfo
                                 else "")
      )
...
object PathOrdering extends Ordering[TPath]:
  ...
  final def compare(p1: TPath, p2: TPath): Int =
    if p1.length < p2.length then -1
      ...
      if result == 0 then compare(p1.tail, p2.tail)
      else result
